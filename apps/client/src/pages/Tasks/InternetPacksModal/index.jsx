...
function Index({ internetPacksModalOpan, closeInternetPacksModal }) {
  ...
  const deleteInternetPack = async id => {
    const response = await fetch(`https://app.digitask.store/services/services/internet_packs/${id}/`, {
      method: "DELETE"
    });
    if (response.ok) {
      fetchInternetPacks();
    ...
    <>
      ...
      >
        <EditInternetPacksModal
          openModal={openModal}
          goCloseModal={goCloseModal}
          editedPack={editedPack}
          fetchInternetPacks={fetchInternetPacks}
        />
        <div className="newPackButton">
          <Button onClick={() => goOpenModal(null)}>Yeni internet paketi</Button>
        </div>
